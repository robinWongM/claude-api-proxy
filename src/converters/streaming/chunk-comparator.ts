...
export class ChunkComparator {
  ...
  private toolCallState = new Map<number, { id?: string; name?: string; args: string }>();
  ...
  processChunk(chunk: OpenAIStreamChunk, controller: TransformStreamDefaultController<Uint8Array>): void {
    ...
    if (this.previousChunk === null) {
      ...
      this.handleChunkTransition(this.previousChunk, currentChunk, controller, chunk);
    }
  ...
  finalize(controller: TransformStreamDefaultController<Uint8Array>): void {
    ...
    } else if (this.activeBlockType === 'tool' && this.activeToolIndex !== null) {
  ...
  private extractChunkContent(chunk: OpenAIStreamChunk): ChunkContent {
    ...
    if (delta.tool_calls && delta.tool_calls.length > 0) {
      ...
      content.activeToolIndex = typeof toolCall.index === 'number' ? toolCall.index : 0;
      ...
      const prev = this.toolCallState.get(idx) || { id: undefined, name: undefined, args: '' };
  ...
  private handleFirstChunk(current: ChunkContent, controller: TransformStreamDefaultController<Uint8Array>, _chunk: OpenAIStreamChunk): void {
  ...
  private handleChunkTransition(previous: ChunkContent, current: ChunkContent, controller: TransformStreamDefaultController<Uint8Array>, _chunk: OpenAIStreamChunk): void {
    if (previous.hasContent && current.hasContent) {
      ...
      this.activeBlockType = 'text';
      this.activeToolIndex = null;
    } else if (!previous.hasContent && !previous.hasToolCalls && current.hasContent) {
      ...
      this.activeBlockType = 'text';
      this.activeToolIndex = null;
    } else if (!previous.hasContent && !previous.hasToolCalls && current.hasToolCalls) {
  ...
  private emitToolCallStart(current: ChunkContent, controller: TransformStreamDefaultController<Uint8Array>): void {
    const blockIndex = current.activeToolIndex! + 1;
    const toolState = this.toolCallState.get(current.activeToolIndex!);
    const id = current.toolId || toolState?.id || `toolu_${current.activeToolIndex}`;
  ...
  private emitToolCallDelta(current: ChunkContent, controller: TransformStreamDefaultController<Uint8Array>): void {
    const blockIndex = current.activeToolIndex! + 1;
    controller.enqueue(new TextEncoder().encode(formatSSEWithEvent('content_block_delta', {
      type: 'content_block_delta',
      index: blockIndex,
