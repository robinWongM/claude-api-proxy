...
describe("Index Module", () => {
	...
	describe("Model name converter", () => {
		test("should convert model names correctly", () => {
			expect(
				modelNameConverter.anthropicToOpenAI("claude-3-5-sonnet-20241022"),
			).toBe("claude-3.5-sonnet");
			expect(modelNameConverter.openAIToAnthropic("claude-3.5-sonnet")).toBe(
				"claude-3-5-sonnet-20241022",
			);
		});

		test("should handle unknown models", () => {
			expect(modelNameConverter.anthropicToOpenAI("unknown-model")).toBe(
				"unknown-model",
			);
			expect(modelNameConverter.openAIToAnthropic("unknown-model")).toBe(
				"unknown-model",
			);
		});
	});
