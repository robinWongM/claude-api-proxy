...
function buildTargetHeaders(
	...
	if (Array.isArray(anthropicRequest.system)) {
		hasCache = anthropicRequest.system.some(
			(s) => "cache_control" in s && !!s.cache_control,
		);
	}
	if (!hasCache)
		hasCache = anthropicRequest.messages.some((m) => !!m.cache_control);
	if (!hasCache && anthropicRequest.tools)
		hasCache = anthropicRequest.tools.some((t) => !!t.cache_control);
...
export async function handleMessagesProxy(
	...
	const debugRequestId: string | null = null;
	...
	try {
		...
		const openaiResponse =
			(await targetResponse.json()) as OpenAIChatCompletionResponse;
		if (config.enableDebug && debugRequestId) {
			const dir = join(config.debugDir, debugRequestId);
			await ensureDir(dir);
			await writeFile(
				join(dir, `openai-response.json`),
